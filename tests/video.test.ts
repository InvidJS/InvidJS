...
describe("Video fetch test", () => {
    test("Video must be fetched correctly.", async () => {
        ...
        expect(await InvidJS.fetchVideo(instance[0], "dQw4w9WgXcQ")).not.toBeUndefined();
    }, 30000)

    test("Must be able to fetch minimal video.", async () => {
        let instance = await InvidJS.fetchInstances({url: "https://y.com.sb"});
        expect((await InvidJS.fetchVideo(instance[0], "dQw4w9WgXcQ", {type: FetchTypes.Minimal})).id).not.toBeUndefined();
    }, 30000)

    test("Must be able to fetch basic video.", async () => {
        let instance = await InvidJS.fetchInstances({url: "https://y.com.sb"});
        expect((await InvidJS.fetchVideo(instance[0], "dQw4w9WgXcQ", {type: FetchTypes.Basic})).formats).not.toBeUndefined();
    }, 30000)

    test("Must be able to fetch full video.", async () => {
        let instance = await InvidJS.fetchInstances({url: "https://y.com.sb"});
        expect((await InvidJS.fetchVideo(instance[0], "dQw4w9WgXcQ", {type: FetchTypes.Full})).description).not.toBeUndefined();
    }, 30000)
